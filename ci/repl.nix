...
# is a value to splice. It resolves the acceptance run's own — this flake's `gen-scope`, the prelude
# beneath it and the algebra it declares directly — so what the REPL hands back is the surface the
# suite tests and not a second, differently-pinned one. That is also why it needs `--impure`.
let
