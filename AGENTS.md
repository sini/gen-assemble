...
| Declare that a node exists                                                | A contribution's `vertices` — the DECLARED-MEMBERSHIP carrier, and the ONLY thing that declares a node a RELATION may name (`decls`/`types` content keys sit outside the check). A list of ids, combined across contributions by SET UNION (one isolated vertex per id, overlaid, so the graph monoid supplies the union and no ordering rule is owed). A declared id no relation contains is a root; an id a graph carries as a vertex is PRESENCE, not declaration                                                                                                                                                                                                 |
| Work out why an edge was refused for an undeclared endpoint               | The DECLARED-MEMBERSHIP REFUSAL in `lib/contribute.nix`. Every edge of every family a contribution carries — `parentGraph` under the label `P`, `importGraph` under `I`, and each `edgeGraphs` label — has **both** endpoints checked against the declared member union, at the union boundary and unconditionally. The union is GLOBAL, so one layer may relate what another declared; the diagnostic names the layer whose contribution carried the edge, the label, and the missing id. `undeclaredEndpoints` renders those facts as data (published from `lib/contribute.nix`, deliberately not on the nine-name library surface). It does **not** ride `strict` |
