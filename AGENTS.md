...
| Declare that a node exists                                                | A contribution's `vertices` — the DECLARED-MEMBERSHIP carrier. A list of ids, combined across contributions by SET UNION (one isolated vertex per id, overlaid, so the graph monoid supplies the union and no ordering rule is owed). A declared id no relation contains is a root. **Carried, not validated:** nothing relates the declared members to the ids the edge graphs mention, in either direction |
