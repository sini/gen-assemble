...
#
# THE PIN SOURCE IS `ci/flake.lock`, WHICH IS WHY THIS ENTRY CAN EXIST AT ALL. gen-assemble declares
...
#
# AND IT IS STATED OVER PATHS, NOT NAMES. Neither `gen-prelude` nor `gen-algebra` is a root input of
# that lock; the acceptance run reaches both THROUGH gen-scope (`ci/flake.nix`:
# `gen-scope.inputs.gen-prelude.lib` and `gen-scope.inputs.gen-schema.inputs.gen-algebra.lib`), so
# the defaults transcribe the wiring that is already there rather than adding a second pin of it.
...
in
{
  ...
  # formal NAME and its resolved PATH are both in scope is this file's argument TO `wire`, not to
  # `./lib`: `wire` RECEIVES that attrset, and `./lib` sees only whatever `wire` chooses to hand
  # it — here, the default `args: import ./lib args,`, which is the only thing that makes the two
  # coincide. A cell injecting `dep = segs: segs` alongside `wire = args: args` reads this shim's
  # own formal-to-path map straight off, with nothing fetched and no path restated by hand. It is
  # a widening and so breaks no caller — there is no `...` here, and no caller passes a name this
